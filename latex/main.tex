...
from this change alone). Stabilized spans are cut into ten-second windows,
round-robin per flight, every third held out; the autonomous flight is
excluded (its lateral commands bypass the recorded sticks --- the rudder
channel is constant while the aircraft banks).
\paragraph{Results.} Held-out five-second free-run position error 3.4\,m
over 11 windows (52k training samples); free runs fly complete laps. The
