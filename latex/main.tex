...
The nominal grey-box row evaluates the coded 3-DOF aircraft model without the hidden nonlinear aerodynamic residuals. It is a no-fit baseline and appears in the result figures as the reference performance level.
...
\paragraph{OEM-SS.}
The output-error single-shooting row estimates fixed aerodynamic parameters by optimizing trajectory error through the longitudinal model. The implementation uses CasADi/IPOPT and is the main deterministic fixed-structure aircraft identification baseline for direct-state data \cite{jategaonkar2006flight,Morelli2002SIDPAC}.
...
The PINN-style rows \cite{raissi2017physics,lagaris1998artificial} use flight-dynamics-level residuals, not full CFD or Navier--Stokes constraints. The coefficient-closure row learns aerodynamic closures, while the hidden-elevator row tests whether a neural model can infer a hidden command correction in SAFE-like cases.

\paragraph{UDE-Residual and UDE-HiddenControl.}
The UDE rows \cite{rackauckas2020universal} retain the known longitudinal dynamics and learn residual terms. The residual row targets missing aerodynamics, while the hidden-control row targets unknown input reshaping from a proprietary SAFE-like controller.
...
\begin{align}
    ...
    \label{eq:CM}
\end{align}
where $\hat{q} = \bar{c}Q/(2V)$ is the conventional non-dimensional pitch
rate and $\bar{c}$ is the mean aerodynamic chord, so all pitching-moment
derivatives are standard non-dimensional stability derivatives.
...
\Delta C_M &=
-0.033\alpha_r^3
+0.033\alpha_r\delta_r
+0.022\tanh(q_r)|\alpha_r|
+0.014\sin(2\gamma)
-g_s\left(0.42+3.0\epsilon_\alpha\right)s_\alpha
-0.10\,g_s\tanh(\delta_r),
\end{align}
...
The pitching moment uses the standard convention
$M = C_{M}\bar{q}S\bar{c}$ with the non-dimensional pitch rate
$\hat{q} = \bar{c}Q/(2V)$, so $C_{M_Q}$ and the other pitching-moment
derivatives in \eqref{eq:CM} are dimensionless and directly comparable to
tabulated stability derivatives. The $\bar{c}/(2V)$ rate scaling also gives
the model its physically correct speed-dependent pitch damping; the resulting
short-period mode of the nominal aircraft is approximately $1.1$~Hz with
damping ratio $0.74$ at the $15$~m/s trim condition, above a lightly damped
phugoid.
...
Mean chord $\bar{c}$ & $0.18$ m & \texttt{Aircraft.chord} \\
...
Nominal $C_{M0}, C_{M\alpha}$ & $0.05,\;-0.60$ & \texttt{NominalAero} \\
Nominal $C_{MQ}, C_{M\delta_e}$ & $-12.0,\;0.60$ & \texttt{NominalAero} \\
Sample time $\Delta t$ & $0.01$ s & locked 100 Hz mocap rate \\
Trajectory duration & $40$ s ($10$ s for unstabilized aggressive/sweep records) & \texttt{SimulationConfig.duration} \\
...
bands: approximately $V\in[8.5,26]$ m/s,
$\alpha\in[-4^\circ,18^\circ]$, $\gamma\in[-20^\circ,18^\circ]$, and
$q\in[-55,50]^\circ$/s over the union of regimes; the unstabilized
aggressive and sweep records are short ($10$~s) flight-test maneuvers because
the realistic lightly damped phugoid cannot be flown open loop for long
periods. The validation split shifts
...
\begin{align}
    ...
    \widehat{\mathbf{C}}_k
        &= \left[\frac{\widehat{L}_k}{\bar{q}_k S},
                 \frac{\widehat{D}_k}{\bar{q}_k S},
                 \frac{\widehat{M}_k}{\bar{q}_k S \bar{c}}\right]^T.
...
\begin{align}
    ...
    [\widehat{C}_{M0},\widehat{C}_{M\alpha},
     \widehat{C}_{MQ},\widehat{C}_{M\delta_e}]^T
        ...
        -[1,\widetilde{\alpha}_k,\hat{q}_k,\delta_{e,k}]\beta\right)^2,
        \label{eq:code_equation_error}
\end{align}
with $\hat{q}_k = \bar{c}\widetilde{Q}_k/(2\widetilde{V}_k)$.
The fitted vector is clipped to the code bounds
$[-0.5,0,0,0,-1,-5,-50,-3]\le\theta\le
[0.5,6,0.2,0.5,1,3,0,3]$. This matches
\texttt{infer\_coefficients} and \texttt{fit\_equation\_error}.

\paragraph{Output-error multiple shooting.}
The coded OEM estimates one global aerodynamic parameter vector together with
multiple-shooting node states for each selected training trajectory. Each
trajectory is split into $N_s$ segments with free node states
$\{\mathbf{x}_{s}^{(r)}\}$; the objective combines weighted measurement
residuals along each simulated segment with soft continuity residuals at the
segment boundaries:
\begin{equation}
    \min_{\theta,\{\mathbf{x}_{s}^{(r)}\}}
    ...
    \sum_{r,s,j}
    \left(
    \frac{\hat{x}_{\mathrm{end},j}^{(r,s)}-x_{s+1,j}^{(r)}}{\sigma_{c,j}}
    \right)^2,
    \qquad
...
bounded parameters, bounded node states, a selected number of trajectories,
a fit stride, and six segments per trajectory; the IPOPT return status is
recorded with the result so unconverged solves are flagged rather than
reported silently. Multiple shooting is required here because the misspecified
initial guess is statically unstable in pitch and a single-shooting rollout
diverges over the record length. This is the implementation in
...
\begin{align}
    ...
    &=
    \frac{
    J_y\dot{\widetilde{Q}}_k/(\bar{q}_kS\bar{c})
    -C_{M0}-C_{M\alpha}\widetilde{\alpha}_k-C_{MQ}\hat{q}_k}
    {C_{M\delta_e}}.
\end{align}
...
elevator pulses, throttle excursions, and brief pull-and-release stall probes
to leave the near-trim region. The SAFE-on variants receive similarly aggressive pilot
...
$\dot\alpha$ terms. A further interpretation caveat from the
manufacturer's manual: the ``manual'' (Experienced-mode) segments remove
the SAFE envelope limits but the receiver's AS3X gyro stabilization remains
active, so the fitted rate-damping coefficients are \emph{effective
closed-loop} damping including gyro feedback rather than bare-airframe
aerodynamic derivatives; predictions are unaffected because the
augmentation is always present in flight.
...
Validation scores are full-state mean NRMSE over open-loop rollouts (lower is
better); ``synthetic'' results average the four simulated scenarios and
...
one audit violation (the 3-DOF suite defaulting to the post-controller
...
6-DOF rollouts clamp speed and rates; 3-DOF rollouts freeze on leaving a
generous physical box, keeping one clipped sample so the diverged flag
triggers (previously an exponential blowup posted an $\alpha$-RMSE of
1{,}160 radians as a finite, unflagged score).
...
weakly identifiable over the narrow $C_L$ range).
...
match the real airframe (opposite the synthetic convention). Not run on
...
ridge map, not a trained network (the 3-DOF tier carries the genuine
ODE-in-the-loop torch UDE).
...
under three names; their notes say so --- scores are identical by
construction at 0.260/0.280 synthetic, 0.693/0.247 real),
Variational-Mocap (smoothed weak-form ridge; 3.71/3.67 synthetic),
PINN-Closure (a sparsified copy of the UDE weights), and NN-Surrogate (a
random-feature closed form) are labeled \emph{placeholder} in every
artifact, are excluded from headline claims, and exist to reserve the rows
until genuine implementations land (filter-error method, CIFER-style
frequency fitting, and a trained 6-DOF network are the roadmap items).
...
\paragraph{Results.} Held-out five-second free-run position error 2.98\,m
over 11 windows (52k training samples; 3.41\,m before the simulation-error
refinement, against a 21.9\,m naive constant-turn baseline); free runs fly
...
deflection buys a moment scaling with dynamic pressure, so at low speed
the loop needs deflections beyond the surface throws to track the clipped
bank command within maneuver timescales, while speed-independent
disturbance moments (propeller torque and slipstream at high power)
consume a growing share of the moment budget. A
...
dynamics against the nominal airframe.
...
\subsection{3-DOF Implementation Decisions}
The 3-DOF formulations appear in the supplemental sections; decisions worth
review: the suite OEM is CasADi multiple shooting (soft continuity
