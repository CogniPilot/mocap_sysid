...
The 6-DOF benchmark is the repository's nonlinear coupled-dynamics aircraft problem. The state is
\[
\mathbf{x}=
\begin{bmatrix}
...
This section keeps equation-level details for the implemented method families
whose benchmark results are generated by the repository.
...
integrating the aircraft model over the complete experiment horizon.
...
\[
    ...
    \qquad k=0,\ldots,N-1.
\]
In the motion-capture setting,
the latent trajectory is passed through the pose measurement model
\[
\hat{\mathbf{y}}_{\mathrm{mocap},k}
=h(\hat{\mathbf{x}}_k)
=
\begin{bmatrix}
\hat{\mathbf{p}}_k^{\top} & \hat{\mathbf{q}}_k^{\top}
\end{bmatrix}^{T},
\]
the predicted inertial position and attitude quaternion. This output-error
formulation avoids using numerically differentiated mocap data as the
dependent variable. The negative log-likelihood
...
\right],
\]
over both the model parameters and the smoothing distribution. In Dutra's aircraft formulation \citep{dutra_variational_2025}, the assumed density is chosen to preserve a Markov structure so that the objective remains sparse and tractable. This makes VI a natural candidate for the mocap-output problem considered here: the measured outputs are position and attitude, the flight states are latent, and turbulence or unmodeled disturbances can be represented as process noise rather than being forced into measurement residuals.
...
candidate nonlinear functions. For the benchmark, the dependent variable for a
fully black-box SINDy model would be the smoothed derivatives of the dynamic
states (body velocities and angular rates), with the candidate library
evaluated on the invariant features and pilot commands. The standard SINDy model is
\begin{equation}
    \dot{X} = \Theta(X,U)\,\Xi,
...
body velocities and angular rates. Hidden actuator
...
\[
\widehat{C}_{i,\phi}=\mathcal{N}_{i,\phi}(\mathbf{z}),\qquad
i\in\{X,Y,Z,l,m,n\},
\]
evaluated on the invariant features $\mathbf{z}$, rather than treating a low-order coefficient expansion as the true physics. The corresponding dimensional forces and moments scale with dynamic pressure $\bar{q}=\frac{1}{2}\rho V^2$ and the reference geometry, and the differential-equation residual $\mathbf{r}_{\mathrm{dyn}}$ enforces the rigid-body force and moment balance on the six dynamic states. The attitude and position kinematics are not aerodynamic discovery targets and should preferably be enforced by the state parameterization itself; if they are not hard-coded, they can be included as high-weight residuals, but this is a numerical convenience rather than a claim that the kinematics are uncertain physics.
...
measurements mid-rollout, and measured states reappear only in scoring.
Rollouts use the pilot command $u_{\mathrm{cmd}}$; the post-controller
actuator signal $u_{\mathrm{act}}$ exists in the synthetic datasets only as
a documented oracle diagnostic.
...
Rollouts clamp speed and rates and keep clipped samples so the diverged
flag triggers honestly instead of letting an exponential blowup post a
large but finite, unflagged score.
...
\paragraph{Approach.} The hidden inner loop is modeled per attitude axis as
$\delta=K_p(\mathrm{sat}(c\cdot\mathrm{stick},
\pm\ell)-\mathrm{att})-K_d\cdot\mathrm{rate}+b$ followed by a
first-order surface lag (rudder: stick gain, yaw-rate gain, bias), and is
identified against the \emph{fitted} grey-box airframe --- never the nominal
model, whose control-effectiveness rows are wrong for this airframe.
Identification is staged. (1) \emph{Structure inference from the
closed-loop fit}: on the body-rate rows the $3\times3$ surface
effectiveness of the fitted grey-box is invertible, so the static law
implied by the identified closed-loop dynamics is recovered exactly from
$B\,K = A_{\mathrm{cl}}-A$; restricting the feedback to states the
speed-blind SAFE unit can sense (attitude and rates) attributes the
remainder of the closed-loop change to stabilized-regime airframe mismatch
rather than control action. (2) \emph{Effective-surface initialization}:
inverse dynamics through the fitted moment model recovers per-sample
deflections --- scalar in $K_{Me}$ for pitch and a coupled $2\times2$
lateral solve in $(K_{L\delta_a},K_{L\delta_r};K_{N\delta_a},K_{N\delta_r})$,
because the fitted airframe carries roughly fifty times more roll authority
in the dihedral/rudder path than in the ailerons, which makes per-axis
scalar inversion structurally wrong and the recovered aileron channel
weakly identified. A staged grid over $(c,\ell)$ wraps a Huber regression
for $(K_p,K_d,b)$. (3) \emph{Joint closed-loop refinement}: the controller
parameters, surface lags, and corrections to all 22 airframe parameters ---
each correction penalized in units of its manual-fit Cram\'er--Rao standard
deviation, a Gaussian prior centered on the manual estimate --- are
optimized by simulation error over 5\,s rollouts of the composed system on
the train stabilized windows, the same horizon the validation metric uses.
The law is proportional--derivative only --- no integral action ---
consistent with three observations: the transmitter carries manual elevator
trim buttons (integral action would make them redundant in stabilized
mode), the fitted closed-loop residuals show no unexplained slow mode, and
attitude integrators wind up in exactly the low-authority regime this
airframe occupies. A ten-second ground test discriminates directly: hold
the airframe tilted --- a ramping surface deflection indicates integral
action, a steady deflection pure PD.
\paragraph{Implementation.}
\texttt{models/aircraft6dof/safe\_controller.py}; vectorized batched RK4
rollouts of the Euler-state grey-box (verified to machine precision against
the CasADi reference) make the joint trust-region solve tractable; the
stabilized window cuts and round-robin train/validation splits are shared
with the closed-loop fit through
\texttt{models/aircraft6dof/segmentation.py}, so the composed model and the
direct fit score on identical held-out windows.
\paragraph{Results.} The structure inference supports the attitude-command
PD form but shows that sensed attitude/rate feedback explains only about
40\% of the rate-row closed-loop change; the rest is regime mismatch
between the manual-maneuver airframe fit and stabilized-lap flight, which
no static feedback law can absorb. The composed grey-box+controller
improves from 16.8\,m (effective-surface staged initialization; the
previous nominal-airframe identification scored 13.6\,m on the same
metric) to 10.8\,m held-out 5\,s position error after the joint
refinement, with an elevator lag near 50\,ms and the aileron channel
effectively decoupled (consistent with its negligible fitted roll
authority). The direct closed-loop linear fit remains better (2.98\,m) and
stays the stabilized-segment prediction model: the gap quantifies the
closed-loop dynamics that static-feedback composition with a
manual-regime airframe cannot represent, and is the standing motivation
for a regime-aware airframe (per-flight thrust scale, stabilized-trim
corrections) rather than a richer controller.

\subsection{Ground-Roll and Ground-Effect Models}
\paragraph{Approach.} The recorded sessions include 133\,s of tracked
ground rolling (taxi and takeoff rolls), segmented automatically by the
altitude hysteresis. A planar rolling model with states
$(p_n,p_e,\psi,V)$ is identified on those spans:
$\dot V = k_T (T_{\max}/m)\,\delta_T^{1.45} - \mu g - c_v V^2$ and
$\dot\psi = (k_s\,\delta_r + k_0)\,V$, with position integrating along
heading. Thrust uses the nominal map as a stated prior with a fitted scale
(the near-constant ground throttle cannot separate thrust from rolling
resistance; the standalone ground-roll study reports the sensitivity), the
heading rate scales with ground speed as wheel steering does, and $k_0$
absorbs wheel-alignment and thrust asymmetry. The ground spans are cut into
the same 10\,s windows with the same round-robin held-out assignment as the
stabilized windows, the parameters are fitted by simulation error on the
train windows, and the held-out windows score 5\,s free-run position error
--- the identical protocol and metric as the airborne models. The
ground-effect band (between the ground and 0.65\,m altitude) holds only
13\,s of rotation/flare transition, too little for a validated rollout
model; instead the in-band lift and drag increments are identified by
equation error against the fitted grey-box, as the difference between the
band's force-coefficient residuals and the airborne-manual reference.
\paragraph{Implementation.}
\texttt{models/aircraft6dof/ground\_model.py}; the explorer free runs
anchored on the ground integrate the planar model until the recorded
liftoff and then hand the rolled position, heading, and speed to the
selected airframe method, mirroring the SAFE-segment handoff.
\paragraph{Results.} Held-out 5\,s position error 2.7\,m over the ground
windows, against 3.4\,m for a hold-position baseline and 11.4\,m for
constant-velocity extrapolation; the fitted thrust scale (1.18) is
consistent with the nominal map, rolling resistance $\mu=0.28$ reflects
wheel scrub on the gym floor, and the steering trim $k_0$ captures the
airframe's consistent left pull. The ground-effect contrast is
statistically significant but reads as a lift \emph{deficit}
($\Delta C_L=-0.054\pm0.002$) with a drag excess
($\Delta C_D=+0.044\pm0.003$): the band is dominated by high-incidence
rotation and flare maneuvers, so the classical ground-effect augmentation
is not resolvable from it, and the increments are reported as a band
diagnostic rather than applied to the prediction models.
