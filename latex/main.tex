...
with no further state resets: manual segments use the recorded sticks
re-referenced by the per-flight trim bias. Stabilized segments use a
\emph{directly identified closed-loop model} (state and stick to next state,
ridge-fitted on every tracked stabilized sample of the session) rather than
composing the bare airframe with the inverse-dynamics controller estimate:
the closed loop is abundantly excited by the lap data and is exactly what
stabilized segments replay, and on five-second stabilized free-runs it
reduces the full-state NRMSE roughly four-fold relative to the
controller-decomposition route (about 0.3--0.5 versus 1.4--1.7). The
...
$b/2V$ and $\bar c/2V$ rate terms) by output-error multiple shooting: every
...
RK4 step for segment scores and free-run predictions.
