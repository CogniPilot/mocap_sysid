...
export function buildModelicaFlightCatalog(models, externalEntries = []) {
  if (!models) return [];
  const catalog = [];
  const push = (entry) => { if (entry) catalog.push(entry); };

  const greyboxMo = models.greybox?.modelica;
  const ctrlMo = models.safe_controller?.modelica;
  ...
  for (const entry of externalEntries) push(entry);
  return catalog;
}
...
export async function createModelicaFlightRunner(entry, source, options = {}) {
  ...
  return {
    modelName,
    supportsSafeToggle: inputNames.has("enable_safe"),
    reset() {
      stepper.reset();
